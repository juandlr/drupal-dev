(function ($) {
  ...
  function aviaCalcContentWidth() {
    ...
    var win = $(window),
      ...
      outer = $(width_select), outerParent = outer.parents('div:eq(0)'),
      the_main = $(width_select + ' .container:first'), css_block = '',
      calc_dimensions = function () {
        var css = '', w_12 = Math.round(the_main.width()),
  ...
  function AviaScrollSpy(element, options) {
    ...
    self.selector = (self.options.target || ((href = $(element).attr('href')) && href.replace(/.*(?=#[^\s]+$)/, '')) //strip for ie7
      || '');
  ...
  AviaScrollSpy.prototype = {
    ...
    constructor: AviaScrollSpy, checkFirst: function () {
      ...
      $targets = this.$body
        ...
        .map(function () {
          var $el = $(this), href = $el.data('target') || $el.attr('href'),
            hash = this.hash, hash = hash.replace(/\//g, ''),
    ...
    , process: function () {
      ...
      var scrollTop = this.$scrollElement.scrollTop() + this.options.offset,
        ...
        offsets = this.offsets, targets = this.targets,
  ...
  $.fn.avia_scrollspy.calc_offset = function () {
    var offset_1 = (parseInt($('.html_header_sticky #main').data('scroll-offset'), 10)) || 0,
      ...
      offset_4 = 1, offset_5 = parseInt($('html').css('margin-top'), 10) || 0,
  ...
  $.fn.avia_scrollspy.defaults = {
    offset: $.fn.avia_scrollspy.calc_offset(), applyClass: 'current-menu-item',
  };

  function avia_site_preloader() {
    var win = $(window), preloader_active = $('html.av-preloader-active'),
      pre_wrap;
  ...
  function AviaBrowserDetection(outputClassElement) {
    if (typeof ($.browser) !== 'undefined') {
      ...
      if ($.browser.name) {
        bodyclass += ' avia-' + $.browser.name + ' avia-' + $.browser.name + '-' + version + ' ';
      }
  ...
  $.fn.avia_html5_activation = function (options) {
    ...
    this.each(function () {
      var fv = $(this), id_to_apply = '#' + fv.attr('id'),
        posterImg = fv.attr('poster');

      fv.mediaelementplayer({
        // if the <video width> is not specified, this is the default
        defaultVideoWidth: 480, // if the <video height> is not specified, this is the default
        defaultVideoHeight: 270, // if set, overrides <video width>
        videoWidth: -1, // if set, overrides <video height>
        videoHeight: -1, // width of audio player
        audioWidth: 400, // height of audio player
        audioHeight: 30, // initial volume when the player starts
        startVolume: 0.8, // useful for <audio> player loops
        loop: false, // enables Flash and Silverlight to resize to content size
        enableAutosize: false, // the order of controls you want on the control bar (and other plugins below)
        features: ['playpause', 'progress', 'current', 'duration', 'tracks', 'volume'], // Hide controls when playing and mouse is not over the video
        alwaysShowControls: false, // force iPad's native controls
        iPadUseNativeControls: false, // force iPhone's native controls
        iPhoneUseNativeControls: false, // force Android's native controls
        AndroidUseNativeControls: false, // forces the hour marker (##:00:00)
        alwaysShowHours: false, // show framecount in timecode (##:00:00:00)
        showTimecodeFrameCount: false, // used when showTimecodeFrameCount is set to true
        framesPerSecond: 25, // turns keyboard support on and off for this instance
        enableKeyboard: true, // when this player starts, it will pause other players
        pauseOtherPlayers: false, poster: posterImg, success: function (mediaElement, domObject, instance) {
  ...
  function avia_hover_effect(container) {
    ...
    elements.each(function (e) {
      ...
      var url = link.attr('href'), span_class = 'overlay-type-video',
        opa = link.data('opacity') || 0.7, overlay_offset = 5,
      ...
      link.on('mouseenter', function (e) {
        var current = link.find('img:first'), _self = current.get(0),
          outerH = current.outerHeight(), outerW = current.outerWidth(),
          pos = current.position(), linkCss = link.css('display'),
          overlay = link.find('.image-overlay');
  ...
  (function ($) {
    $.fn.avia_smoothscroll = function (apply_to_container) {
      ...
      var the_win = $(window), $header = $('#header'),
        $main = $('.html_header_top.html_header_sticky #main').not('.page-template-template-blank-php #main'),
        $meta = $('.html_header_top.html_header_unstick_top_disabled #header_meta'),
        $alt = $('.html_header_top:not(.html_top_nav_header) #header_main_alternate'),
        menu_above_logo = $('.html_header_top.html_top_nav_header'),
        shrink = $('.html_header_top.html_header_shrinking').length,
        frame = $('.av-frame-top'), fixedMainPadding = 0,
        isMobile = $.avia_utilities.isMobile,
      ...
      return this.each(function () {
        $(this).click(function (e) {
          ...
          var newHash = this.hash.replace(/\//g, ''), clicked = $(this),
            data = clicked.data();
          ...
          if (newHash != '' && newHash != '#' && newHash != '#prev' && newHash != '#next' && !clicked.is('.comment-reply-link, #cancel-comment-reply-link, .no-scroll')) {
            ...
            if (container.length) {
              var cur_offset = the_win.scrollTop(),
                ...
                hash = window.location.hash, hash = hash.replace(/\//g, ''),
                oldLocation = window.location.href.replace(hash, ''),
                newLocation = this, duration = data.duration || 1200,
  ...
  function avia_iframe_fix(container) {
    ...
    iframe.each(function () {
      var current = jQuery(this), src = current.attr('src');
    ...
    var win = jQuery(window),
      ...
      adjust_iframes = function () {
        iframes.each(function () {
          ...
          var iframe = jQuery(this), parent = iframe.parent(),
            proportions = 56.25;
  ...
  (function ($) {
    $.fn.avia_activate_lightbox = function (variables) {
      ...
      var defaults = {
          groups: ['.avia-slideshow', '.avia-gallery', '.av-instagram-pics', '.portfolio-preview-image', '.portfolio-preview-content', '.isotope', '.post-entry', '.sidebar', '#main', '.main_menu'], autolinkElements: 'a.lightbox, a[rel^="prettyPhoto"], a[rel^="lightbox"], a[href$=jpg], a[href$=png], a[href$=gif], a[href$=jpeg], a[href*=".jpg?"], a[href*=".png?"], a[href*=".gif?"], a[href*=".jpeg?"], a[href$=".mov"] , a[href$=".swf"] , a:regex(href, .vimeo\.com/[0-9]) , a[href*="youtube.com/watch"] , a[href*="screenr.com"], a[href*="iframe=true"]', videoElements: 'a[href$=".mov"] , a[href$=".swf"] , a:regex(href, .vimeo\.com/[0-9]) , a[href*="youtube.com/watch"] , a[href*="screenr.com"], a[href*="iframe=true"]', exclude: '.noLightbox, .noLightbox a, .fakeLightbox, .lightbox-added, a[href*="dropbox.com"]',
        },

        options = $.extend({}, defaults, variables),

        av_popup = {
          type: 'image', mainClass: 'avia-popup mfp-zoom-in', tLoading: '', tClose: '', removalDelay: 300, //delay removal by X to allow out-animation
          closeBtnInside: true, closeOnContentClick: false, midClick: true, fixedContentPos: false, // allows scrolling when lightbox is open but also removes any jumping because of scrollbar removal
          ...
          gallery: {
            ...
            tPrev: '', tNext: '', tCounter: '%curr% / %total%', enabled: true, preload: [1, 1], // Will preload 1 - before current, and 1 after the current image
          },
  ...
  (function ($) {
    $.fn.aviaMegamenu = function (variables) {
      var defaults = {
        modify_position: true, delay: 300,
      };

      var options = $.extend(defaults, variables), win = $(window);

      return this.each(function () {
        var the_html = $('html:first'), main = $('#main .container:first'),
          left_menu = the_html.filter('.html_menu_left, .html_logo_center').length,
          isMobile = $.avia_utilities.isMobile, menu = $(this),
          ...
          parentContainerWidth = parentContainer.width(), delayCheck = {},
          mega_open = [];
        ...
        dropdownItems.find('li').addBack().each(function () {
          var currentItem = $(this), sublist = currentItem.find('ul:first'),
            showList = false;
  ...
  $.fn.avia_iso_sort = function (options) {
    return this.each(function () {
      var the_body = $('body'), container = $(this),
        portfolio_id = container.data('portfolio-id'),
        parentContainer = container.parents('.entry-content-wrapper, .avia-fullwidth-portfolio'),
        filter = parentContainer.find('.sort_width_container[data-portfolio-id="' + portfolio_id + '"]').find('#js_sort_items').css({ visibility: 'visible', opacity: 0 }),
        links = filter.find('a'), imgParent = container.find('.grid-image'),
        isoActive = false, items = $('.post-entry', container);
  ...
  function avia_sticky_submenu() {
    var win = $(window), html = $('html:first'),
      header = $('.html_header_top.html_header_sticky #header'),
      html_margin = parseInt($('html:first').css('margin-top'), 10),
      setWitdth = $('.html_header_sidebar #main, .boxed #main'),
      menus = $('.av-submenu-container'),
      bordermod = html.is('.html_minimal_header') ? 0 : 1,
      fixed_frame = $('.av-frame-top').height(), calc_margin = function () {
        ...
        var menu_pos = this.offset().top, top_pos = placeholder.offset().top,
          scrolled = win.scrollTop(), modifier = html_margin, fixed = false;
        ...
        var clicked = $(this), menu = clicked.siblings('.av-subnav-menu');
    ...
    menus.each(function () {
      ...
      if (sticky.length) {
        win.on('scroll', function () { window.requestAnimationFrame($.proxy(check, sticky, placeholder));});
      }
  ...
  function avia_sidebar_menu() {
    ...
    var innerSidebar = $('#header_main'), wrap = $('#wrap_all'),
  ...
  function avia_header_size() {
    ...
    var logo = $('#header_main .container .logo img, #header_main .container .logo a'),
      ...
      isMobile = $.avia_utilities.isMobile, scroll_top = $('#scroll-top-link'),
  ...
  function avia_hamburger_menu() {
    var header = $('#header'), header_main = $('#main .av-logo-container'), //check if we got a top menu that is above the header
      menu = $('#avia-menu'), burger_wrap = $('.av-burger-menu-main a'),
      htmlEL = $('html').eq(0),
      overlay = $('<div class="av-burger-overlay"></div>'),
      overlay_scroll = $('<div class="av-burger-overlay-scroll"></div>').appendTo(overlay),
      inner_overlay = $('<div class="av-burger-overlay-inner"></div>').appendTo(overlay_scroll),
      bgColor = $('<div class="av-burger-overlay-bg"></div>').appendTo(overlay),
      animating = false, first_level = {},
      logo_container = $('.av-logo-container .inner-container'),
      menu_in_logo_container = logo_container.find('.main_menu'),
      cloneFirst = htmlEL.is('.html_av-submenu-display-click.html_av-submenu-clone, .html_av-submenu-display-hover.html_av-submenu-clone'),
      menu_generated = false, set_list_container_height = function () {
        ...
        var list, link, current, subitems, megacolumns, sub_current,
          sub_current_list, new_li, new_ul;
          ...
          } else if (megacolumns.length)	//if we got no normal sublists try megamenu columns and sublists
          {
            ...
            megacolumns.each(function (iteration) {
              var megacolumn = $(this),
                ...
                mega_title_set = false, mega_link = new_li.find('>a'),
                hide_enty = '';
    ...
    $('body').on('mousewheel DOMMouseScroll touchmove', '.av-burger-overlay-scroll', function (e) {
      ...
      var height = this.offsetHeight, scrollHeight = this.scrollHeight,
    ...
    $(document).on('touchmove', '.av-burger-overlay-scroll', function (e) {
      ...
      var differenceY = e.originalEvent.touches[0].clientY - touchPos.Y,
        element = this, top = element.scrollTop,
  ...
  $.AviaAjaxSearch.prototype = {
    ...
    do_search: function (e) {
      var obj = this,
        ...
        action = form.attr('action'), values = form.serialize();
      ...
      $.ajax({
        url: avia_framework_globals.ajaxurl, type: 'POST', data: values, beforeSend: function () {
  ...
  $.AviaTooltip.prototype = {
    ...
    display_tooltip: function (e) {
      var _self = this,
        ...
        element = $(target), text = element.data(this.options.data),
        newTip = element.data('avia-created-tooltip'),
        extraClass = element.data('avia-tooltip-class'),
        attach = this.options.attach == 'element' ? element : this.body,
        offset = this.options.attach == 'element' ? element.position() : element.offset(),
        position = element.data('avia-tooltip-position'),
        align = element.data('avia-tooltip-alignment'), force_append = false;
...
!function (t, e) {'function' == typeof define && define.amd ? define('jquery-bridget/jquery-bridget', ['jquery'], function (i) {return e(t, i);}) : 'object' == typeof module && module.exports ? module.exports = e(t, require('jquery')) : t.jQueryBridget = e(t, t.jQuery);}(window, function (t, e) {
  ...
  var o = Array.prototype.slice, s = t.console,
  ...
  'use strict';
  'function' == typeof define && define.amd ? define('get-size/get-size', [], function () {return e();}) : 'object' == typeof module && module.exports ? module.exports = e() : t.getSize = e();
}(window, function () {
  'use strict';
  ...
  function s(e) {
    if (o(), 'string' == typeof e && (e = document.querySelector(e)), e && 'object' == typeof e && e.nodeType) {
      ...
      var p = a.paddingLeft + a.paddingRight,
        y = a.paddingTop + a.paddingBottom, g = a.marginLeft + a.marginRight,
  ...
  i.extend = function (t, e) {
    ...
    return t;
  }, i.modulo = function (t, e) {return (t % e + e) % e;}, i.makeArray = function (t) {
    var e = [];
    if (Array.isArray(t)) {
      e = t;
  ...
  d.constructor = n, d._create = function () {this._transn = { ingProperties: {}, clean: {}, onEnd: {} }, this.css({ position: 'absolute' });}, d.handleEvent = function (t) {
    ...
    var t = getComputedStyle(this.element),
      ...
      o = t[i ? 'top' : 'bottom'], s = this.layout.size,
    ...
    var t = this.layout.size, e = {}, i = this.layout._getOption('originLeft'),
      ...
      o = i ? 'paddingLeft' : 'paddingRight', s = i ? 'left' : 'right',
      r = i ? 'right' : 'left', a = this.position.x + t[o];
    e[s] = this.getXValue(a), e[r] = '';
    var u = n ? 'paddingTop' : 'paddingBottom', h = n ? 'top' : 'bottom',
      d = n ? 'bottom' : 'top', l = this.position.y + t[u];
    ...
    this.getPosition();
    var i = this.position.x, n = this.position.y, o = parseInt(t, 10),
      s = parseInt(e, 10), r = o === this.position.x && s === this.position.y;
  ...
  }, d.goTo = function (t, e) {this.setPosition(t, e), this.layoutPosition();}, d.moveTo = d._transitionTo, d.setPosition = function (t, e) {this.position.x = parseInt(t, 10), this.position.y = parseInt(e, 10);}, d._nonTransition = function (t) {
  ...
  'use strict';
  'function' == typeof define && define.amd ? define('outlayer/outlayer', ['ev-emitter/ev-emitter', 'get-size/get-size', 'fizzy-ui-utils/utils', './item'], function (i, n, o, s) {return e(t, i, n, o, s);}) : 'object' == typeof module && module.exports ? module.exports = e(t, require('ev-emitter'), require('get-size'), require('fizzy-ui-utils'), require('./item')) : t.Outlayer = e(t, t.EvEmitter, t.getSize, t.fizzyUIUtils, t.Outlayer.Item);
  ...
  var c = s.prototype;
  n.extend(c, e.prototype), c.option = function (t) {n.extend(this.options, t);}, c._getOption = function (t) {
  ...
  }, c._positionItem = function (t, e, i, n, o) {n ? t.goTo(e, i) : (t.stagger(o * this.stagger), t.moveTo(e, i));}, c._postLayout = function () {this.resizeContainer();}, c.resizeContainer = function () {
  ...
  }, c.stamp = function (t) {t = this._find(t), t && (this.stamps = this.stamps.concat(t), t.forEach(this.ignore, this));}, c.unstamp = function (t) {t = this._find(t), t && t.forEach(function (t) {n.removeFrom(this.stamps, t), this.unignore(t);}, this);}, c._find = function (t) {
    if (t) {
      return 'string' == typeof t && (t = this.element.querySelectorAll(t)), t = n.makeArray(t);
    }
  }, c._manageStamps = function () {this.stamps && this.stamps.length && (this._getBoundingRect(), this.stamps.forEach(this._manageStamp, this));}, c._getBoundingRect = function () {
  ...
  }, n.layout = function () {this.isotope.layout.apply(this.isotope, arguments);}, n.getSize = function () {this.isotope.getSize(), this.size = this.isotope.size;}, i.modes = {}, i.create = function (t, e) {
  ...
  return i.compatOptions.fitWidth = 'isFitWidth', i.prototype._resetLayout = function () {
    ...
    var n = this.columnWidth += this.gutter,
      o = this.containerWidth + this.gutter, s = o / n, r = n - o % n,
      a = r && r < 1 ? 'round' : 'floor';
    ...
    var i = e(t), n = this._getElementOffset(t),
      o = this._getOption('originLeft'), s = o ? n.left : n.right,
  ...
  'use strict';
  var i = t.create('masonry'), n = i.prototype,
  ...
  var f = function () {
    function t(t) {
      ...
      var i = h(t).split(' '), n = i[0], o = n.match(/^\[(.+)\]$/),
        s = o && o[1], r = e(s, n), a = d.sortDataParsers[i[1]];
...
!function (a, b) {'function' == typeof define && define.amd ? define('packery/js/rect', b) : 'object' == typeof module && module.exports ? module.exports = b() : (a.Packery = a.Packery || {}, a.Packery.Rect = b());}(window, function () {
  ...
  return b.contains = function (a) {
    ...
    var b = this.x + this.width, c = this.y + this.height, d = a.x + a.width,
      e = a.y + a.height;
    ...
    var c, d = [], e = this.x + this.width, f = this.y + this.height,
  ...
  j._create = function () {
    ...
    var b = this._getItemsForLayout(this.items),
      c = this._getOption('horizontal'), d = c ? 'y' : 'x',
      e = c ? 'height' : 'width', f = c ? 'rowHeight' : 'columnWidth',
      g = c ? 'innerHeight' : 'innerWidth', h = this[f];
    ...
    var e = this._getOption('horizontal'), f = e ? 'rowHeight' : 'columnWidth',
      g = e ? 'height' : 'width';
    ...
    if (i = i && i + this.gutter) {
      ...
      h = this.shiftPacker[g] + this.gutter - a.rect[g], this._addShiftTarget(0, 0, h);
    }
  ...
  var c = a.create('packery'), d = c.prototype,
...
!function () {
  ...
  e.prototype.add = function (t) {
    ...
    var t, e = this.element == this.element.window, i = this.adapter.offset(),
      o = {};
    this.handleScroll(), t = { horizontal: { contextOffset: e ? 0 : i.left, contextScroll: e ? 0 : this.oldScroll.x, contextDimension: this.innerWidth(), oldScroll: this.oldScroll.x, forward: 'right', backward: 'left', offsetProp: 'left' }, vertical: { contextOffset: e ? 0 : i.top, contextScroll: e ? 0 : this.oldScroll.y, contextDimension: this.innerHeight(), oldScroll: this.oldScroll.y, forward: 'down', backward: 'up', offsetProp: 'top' } };
    for (var n in t) {
      var r = t[n];
      for (var s in this.waypoints[n]) {
        var a, l, h, p, u, c = this.waypoints[n][s], d = c.options.offset,
          f = c.triggerPoint, w = 0, y = null == f;
  ...
  }, e.findByElement = function (t) {return o[t.waypointContextKey];}, window.onload = function () {r && r(), e.refreshAll();}, n.requestAnimationFrame = function (e) {
...
jQuery.expr[':'].regex = function (elem, index, match) {
  var matchParams = match[3].split(','), validLabels = /^(data|css):/, attr = {
      method: matchParams[0].match(validLabels) ? matchParams[0].split(':')[0] : 'attr', property: matchParams.shift().replace(validLabels, ''),
    }, regexFlags = 'ig',
